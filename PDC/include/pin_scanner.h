...
public:
    ...
    uint16_t testPin(uint8_t dePin, uint8_t testCount = 3) {
        ...
        for (uint8_t i = 0; i < testCount; i++) {
            ...
            // Send request with proper DE timing
            digitalWrite(dePin, HIGH);  // TX mode
            delayMicroseconds(500);     // Increased: Wait for DE to stabilize
            modbusSerial->write(request, 8);
            modbusSerial->flush();      // Wait for all bytes transmitted
            delay(2);                   // 2ms wait for TX complete
            digitalWrite(dePin, LOW);   // RX mode
            delayMicroseconds(500);     // Wait for DE to stabilize in RX
